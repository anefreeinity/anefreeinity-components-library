...
export const Snackbar: React.FC<SnackbarProps> = ({
    ...
    const snackbarBodyClassNames = `fixed ${property.SnackbarBorderWidth} ${property.SnackbarBorderColor} ${property.SnackbarBackgroundOpacity}
  ${property.SnackbarPosition} transform -translate-x-1/2 max-w-sm md:max-w-md lg:max-w-lg ${property.SnackbarWidth} pr-2 md:pr-4 ${
      title ? `${image ? `pl-1 md:pl-2` : `pl-2 md:pl-3`}` : 'pl-1 md:pl-2'
  } py-1 md:py-2 ${property.SnackbarBackgroundColor} ${property.SnackbarBorderRadius} ${property.SnackbarShadow} flex items-center justify-between transition-all ${
    ...
    return (
        <div className={snackbarBodyClassNames}>
            <div className="flex items-center flex-grow">
                {(staticIcon && <div className={snackbarStaticIconClassNames}>{staticIcon}</div>) ||
                    (!image && property.Icon && (
                        <div className={snackbarStaticIconClassNames}>
                            <FontAwesomeIcon icon={property.Icon} />
                        </div>
                    ))}
                {image && <img src={image} alt="static" className={snackbarImageClassNames} />}
